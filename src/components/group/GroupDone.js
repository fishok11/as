...
const GroupDone = () => {
  ...
  return (
    <div className="g-container__form">
      ...
      <Link to={`/group/${state.group.id}/user-profile/${state.userData.id}`} className="g-link">http://localhost:3000/group/{state.group.id}/user-profile/{state.userData.id}</Link>
      <Typography variant="body2" sx={{ mb: '10px' }}>Не потеряйте ее.</Typography>
